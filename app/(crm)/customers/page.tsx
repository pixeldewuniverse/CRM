...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ q?: string; tag?: string }> }) {
  ...
  if (tag) filters.push(`tag=eq.${encodeURIComponent(tag)}`);
  ...
  return (
    <section>
      ...
      <form className="card mb-4 grid gap-3 md:grid-cols-3">
        <input name="q" placeholder="Search name/phone/email" defaultValue={q} className="rounded border px-3 py-2" />
        <select name="tag" defaultValue={tag} className="rounded border px-3 py-2">
          <option value="">All tags</option>
          <option value="VIP">VIP</option>
          <option value="repeat customer">Repeat Customer</option>
        </select>
        <button className="rounded bg-slate-900 px-3 py-2 text-white">Filter</button>
      </form>
